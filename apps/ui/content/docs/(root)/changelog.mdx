...
The **`input-otp.tsx`** registry component has been removed in favor of **`otp-field.tsx`** (`@creantly/otp-field`), which wraps Base UI OTP Field. See the [OTP Field](/ui/docs/components/otp-field) docs and [migration guide](/ui/docs/radix-migration#otp-field) for API details.
...
```text
Replace the removed @creantly/input-otp / components/ui/input-otp.tsx with @creantly/otp-field / components/ui/otp-field.tsx. Follow the OTP Field docs: use OTPField, OTPFieldInput, and OTPFieldSeparator; use length on the root; remove the old input-otp npm package if present.
```
