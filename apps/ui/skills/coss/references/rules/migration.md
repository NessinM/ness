...
- When only Base UI helpers are needed (`useRender`, `mergeProps`, `CSPProvider`, `DirectionProvider`), prefer `@nesscom/ui/base-ui/*` re-exports over direct `@base-ui/react` dependency.
